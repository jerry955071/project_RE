...
if [ -d tParts ]; then
  ...
  do
    sed -e 's#.*.2bit:##;' tParts/\$tPart.lst \\
      | /data/bin/twoBitToFa -seqList=stdin $seq1Dir stdout \\
        | /data/bin/faToTwoBit stdin tParts/\$tPart.2bit
  done
fi
if [ -d qParts ]; then
  ...
  do
    sed -e 's#.*.2bit:##;' qParts/\$qPart.lst \\
      | /data/bin/twoBitToFa -seqList=stdin $seq2Dir stdout \\
        | /data/bin/faToTwoBit stdin qParts/\$qPart.2bit
  done
fi
_EOF_
    );
...
sub doBlastzClusterRun {
  ...
  $bossScript->add(<<_EOF_
/data/bin/$gensub2 $targetList $queryList gsub jobList
/data/bin/$paraRun
_EOF_
    );
  $bossScript->execute();
